...
async function insert(table:string,data:unknown):Promise<EventResult>{const b=base();if(!b)return{ok:false,status:"BLOCKED_PERSISTENCE_NOT_CONFIGURED"};const r=await fetch(b.url+"/rest/v1/"+table,{method:"POST",headers:{apikey:b.key,Authorization:"Bearer "+b.key,"Content-Type":"application/json",Prefer:"return=representation"},body:JSON.stringify(data),cache:"no-store"});if(r.status===409)return{ok:true,duplicate:true};if(!r.ok)return{ok:false,status:"PERSISTENCE_PROVIDER_ERROR",provider_status:r.status,detail:(await r.text().catch(()=> "")).slice(0,500)};return{ok:true,duplicate:false,data:await r.json().catch(()=>null)}}
