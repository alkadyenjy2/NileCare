...
export async function POST(request:Request){
 const url=new URL(request.url),body=await request.json().catch(()=>null);
 const hmac=request.headers.get("x-paymob-hmac")??url.searchParams.get("hmac")??(body&&typeof body==="object"?String((body as Record<string,unknown>).hmac??""):"");
