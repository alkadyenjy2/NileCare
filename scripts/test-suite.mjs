...
const [d,o,p,w,m,ci,s,n,sp,c]=await Promise.all([
 ...
 file("supabase/002_production_hardening.sql"),file("nilecare-landing/lib/neon.ts"),file("nilecare-landing/lib/supabase.ts"),file("nilecare-landing/app/ContactForm.tsx")
]);
const checks=[
 ...
 /Neon-Connection-String/,/NILECARE_PERSISTENCE_PROVIDER/,/isStage\(from\)/,/try/,/catch/,
];
const values=[d,d,d,d,o,o,p,p,w,w,m,m,s,s,s,s,ci,ci,ci,ci,p,w,n,sp,d,c,c];
checks.forEach((re,i)=>assert.match(values[i],re,"check "+(i+1)+" failed"));
console.log("NILECARE_TEST_SUITE=27/27 PASS");
