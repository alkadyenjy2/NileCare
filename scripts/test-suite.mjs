...
const [d,o,p,w,m,ci,s,n,sp,c,wr]=await Promise.all([
 ...
 file("supabase/002_production_hardening.sql"),file("nilecare-landing/lib/neon.ts"),file("nilecare-landing/lib/supabase.ts"),file("nilecare-landing/app/ContactForm.tsx"),file("nilecare-landing/app/api/whatsapp/webhook/route.ts")
]);
const checks=[
 ...
 /Neon-Connection-String/,/NILECARE_PERSISTENCE_PROVIDER/,/isStage\(from\)/,/try/,/catch/,/INVALID_WHATSAPP_JSON/,
];
const values=[d,d,d,d,o,o,p,p,w,w,m,m,s,s,s,s,ci,ci,ci,ci,p,w,n,sp,d,c,c,wr];
checks.forEach((re,i)=>assert.match(values[i],re,"check "+(i+1)+" failed"));
console.log("NILECARE_TEST_SUITE=28/28 PASS");
